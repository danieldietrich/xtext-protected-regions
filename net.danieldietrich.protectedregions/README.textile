h1(#top). Protected Region Support
...
** Change Protected Region Notation
** Change Switchable Regions Behavior
...
To install the project on your local machine the following tools are needed:
...
The project has to be cloned to your local git repository:

bc. git clone git@github.com:danieldietrich/xtext-protectedregions.git

**Setup Project**

The eclipse project files are generated as follows:

@mvn eclipse:eclipse@

After that the project can be importet into your eclipse workspace.
Assure that _Project Properties/Resource/Text file encoding_ is set to _UTF-8_.
...
If the default parser is configured to parse protected regions (see _Protected Regions_), the start notation is @PROTECTED REGION ID(xxx) START@ and the end notation is @PROTECTED REGION END@. The comments containing these markers and the text between them are protected.

If the default parser is configured to parse inverse protected regions (see _Inverse Protected Regions_), the start notation is @GENERATED ID(xxx) START@ and the end notation is @GENERATED END@. The comments containing these markers and the text between them are generated.

Additionally the parser can be configured to support switchable regions (see _Switchable Regions_). By default the @ENABLED@ keyword is used to enable switched regions. In particular the notation is @PROTECTED REGION ID(xxx) ENABLED START@ and accordingly @GENERATED ID(xxx) ENABLED START@.
