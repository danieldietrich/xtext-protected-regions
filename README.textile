...
*** "Set Target Platform":#setTargetPlatform
...
The user guide is addressed to developers which want to _use_ the Xtext Protected Region Support (PRS) to enhance their existing Xtext generator.

h3(#twoStepSetup). 2-Step Setup
...
bc.. 
Require-Bundle:
 net.danieldietrich.protectedregions.core;bundle-version="1.0.0",
...
_Hint: Xtext PRS is shipped with @BidiJavaIoFileSystemAccess@ and @BidiEclipseResourceFileSystemAccess@._
...
For an in-depth description and more options (like generated regions) please take a look at the "PRS Core documentation":https://github.com/danieldietrich/xtext-protectedregions/blob/master/net.danieldietrich.protectedregions.core/README.textile.
...
_Hint: As of Xtext, instead of @JavaIoFileSystemAccess@ you can use @EclipseResourceFileSystemAccess@._
...
h2(#developerGuide). Developer Guide
...
h4(#importProjects). Import Projects

Maven Tycho is used to build plugins, features and update sites. In Eclipse select Import / Maven / Existing Maven Projects and navigate to the root of xtext-protectedregions cloned before. Download necessary plugins if asked for.

h4(#setTargetPlatform). Set Target Platform

To solve missing dependencies, open @net.dd.prs.target/indigo.target@ (with the _Target Editor_) and select _Set as Target Platform_ (in the upper right corner).
...
h4(#projectLayout). Project Layout

The Xtext PRS consists of the following projects:

table(#tableborders){border:1px solid black}.
|@net.danieldietrich.protectedregionsupport@|Parent maven project. Defining all necessary maven modules and maven plugins.|
...
|@net.danieldietrich.protectedregionsupport.target@|The target platform definition consists of all dependencies of the project and is needed for development of Xtext PRS.|

h3(#buildProcess). Build Process

To build an eclipse update site containing all artifacts, open a shell, jump to projects root and type
