...
%{color:red}Please let us "know":https://github.com/danieldietrich/xtext-protectedregions/issues if there are languages missing.%
...
# The developer of a code generator has to process the generated documents to preserve marked regions.
...
h3(#addLanguages). Additional Languages
...
bc.. IRegionOracle myOracle = new IRegionOracle() {
  @Override public boolean isMarkedRegionStart(String s) {
    return s.startsWith("// code(") && s.endsWith(")-{");
  }
...
h3(#changeSwitchable). Change Switchable Regions Behavior
